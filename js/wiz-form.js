angular.module('wiz', []).directive('wizForm', function () {
  return {
    restrict: 'E',
    template: '<div class="wizard-body">\n\n  <div ng-transclude class="inner">\n  </div>\n\n  <div ng-show="steps[currentStep].show_controls()" class="text-center pagination justify-content-center pt-5 pb-5">\n\n    <div class="wizard-control previous mr-3">\n      <button class="btn btn-secondary previous" ng-click="previousStep()" ng-disabled="currentStep == 0">\n        {{ previousText() || \'Anterior\' }}\n      </button>\n    </div>\n\n    <div class="wizard-control next ml-3">\n      <div class="error">\n        <span ng-show="isError()">{{ message.error }}</span>\n        <button class="btn btn-light next" ng-click="nextStep()" ng-hide="lastStep()">\n          {{ nextText() || \'Siguiente\' }}\n        </button>\n        <button class="btn btn-success finish" ng-click="nextStep()" ng-show="lastStep()">\n          {{ nextText() || \'Guardar\' }}\n        </button>\n      </div>\n    </div>\n\n  </div>\n</div>',
    transclude: true,
    replace: true,
    scope: {
      onFinish: '&',
